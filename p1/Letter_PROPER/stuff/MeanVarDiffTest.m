...
RND = 0;
VD = zeros(size(logspace(-3, -0.5), 2), DATA_SIZE);
MD = zeros(size(logspace(-3, -0.5), 2), DATA_SIZE);
...
VD(RND, :) = abs(Ks{i}.*(Thetas{i}.^2) - diag(Covs{i}).');
MD(RND, :) = abs(Ks{i}.*(Thetas{i}) - Means{i});
